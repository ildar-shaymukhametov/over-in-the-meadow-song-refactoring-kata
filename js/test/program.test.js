import { Program, RandomAnimalSong, RandomSong } from "../src/program";
...
test("random verses", () => {
  expect(new RandomSong(new StubVerseRandomizer()).song()).toEqual(
    "Over in the meadow,\n" +
    "Where the stream runs blue,\n" +
    "Lived an old mother fish\n" +
...
test("random animals", () => {
  expect(new RandomAnimalSong(new StubAnimalRandomizer()).song()).toEqual(
    "Over in the meadow,\n" +
    "In the sand in the sun,\n" +
    "Lived an old mother fish\n" +
    "And her little fish one.\n" +
...
class StubVerseRandomizer {
  constructor() {
    this.verses = [2, 1, 3, 4, 5, 6, 7, 8, 9, 10];
    this.i = 0;
  }
  next() {
    if (this.i <= 10) {
      return this.verses[this.i++];
    } else {
      return null;
    }
  }
